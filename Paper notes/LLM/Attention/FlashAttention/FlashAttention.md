...
![img](./assets/v2-be472360bd3f0323574c3694658a77c6_1440w.jpg)
...
对于输入序列  $Q, K, V \in \mathbb{R}^{N \times d}$ ，其中  $N$  是序列长度， $d$  是token尺寸， $N \gg d$ ，self-attention计算输出  $O \in \mathbb{R}^{N \times d}$ ，计算过程如公式1：


 $$ S = QK^\top \in \mathbb{R}^{N \times N}, \quad P = \text{softmax}(S) \in \mathbb{R}^{N \times N}, \quad O = PV \in \mathbb{R}^{N \times d} \quad (1) $$ 

标准注意力实现将矩阵  $S$  和  $P$  具体化为 HBM，这需要  $O(N^2)$  内存，传统attention的流程如图：

![img](./assets/v2-bda00c885a8d7af4ddc1241a0688b3d1_1440w.jpg)

传统attention的图解如下：每次完整的矩阵运算的复杂度为  $O(N^2)$ ：

![img](./assets/v2-2d885aad40b5dbc6db286a04bfb392da_1440w.jpg)
...
**【回顾softmax】** softmax计算方法：
 ...
 $$ \text{softmax}(x_j) = \frac{e^{x_j}}{\sum_{i=1}^{k} e^{x_i}} \quad (2) $$ 

softmax操作是row-wise的，即每行都算一次softmax，所以需要用到平铺算法来分块计算softmax。

**【safe softmax】** 原始softmax数值不稳定，为了数值稳定性，FlashAttention采用safe softmax，向量  $\in \mathbb{R}$  的safe softmax 计算如下：


 $$ m(x) := \max_i x_i, \quad f(x) := [e^{x_1 - m(x)} \ldots e^{x_B - m(x)}], \quad \ell(x) := \sum_i f(x)_i, \quad \text{softmax}(x) := \frac{f(x)}{\ell(x)} \quad (3) $$ 

同理，则  $x = [x^{(1)} \quad x^{(2)}] \in \mathbb{R}^{2B}$  的softmax也可以通过分解计算：
 ...
 $$ \text{softmax}(x) = \frac{f(x)}{\ell(x)} \quad (4) $$ 

 $f(x)$  和  $\ell(x)$  都可以通过分块计算得出，所以FlashAttention在计算时通过分块将  $Q$ ， $K$ ， $V$  分块后，按块加载到内存中。

### 3.2.2 Recomputation（重新计算）

FlashAttention算法的目标：在计算中减少显存占用，从  $O(N^2)$  大小降低到线性，这样就可以把数据加载到SRAM中，提高IO速度。

解决方案：传统Attention在计算中需要用到  $Q$ ， $K$ ， $V$  去计算  $S$ ， $P$  两个矩阵，FlashAttention引入softmax中的统计量  $(m, \ell)$ ，结合output  $O$  和在SRAM中的  $Q$ ， $K$ ， $V$  块进行计算。

### 3.3.3 算法细节

FlashAttention前向过程

![img](./assets/v2-a3e7f759b3ae9123fd0d7034f2ee6d01_1440w.jpg)
...
![img](./assets/v2-f2b16ae6bab0c6dc85c6cd743541f896_1440w.jpg)
...
![img](./assets/v2-6cd98c507dfe73604b3bc0a7e1051f11_1440w.jpg)

图7

FlashAttention反向过程，反向传播也是通过引入统计量，实现分块计算：

![img](./assets/v2-3474eb857ebe60db2dd49937c4e9ac67_1440w.jpg)
