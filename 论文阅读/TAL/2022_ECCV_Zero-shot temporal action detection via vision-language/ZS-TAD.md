...
　　输入为可学习的文本上下文，<img src="./assets/1859337-20220804205612758-1985684362.png" alt="img" style="zoom:25%;" />是上下文长度。嵌入Gke∈RC'表示第k个类的每个名称的CLIP词汇表中的文本嵌入，背景类的文本嵌入不能直接从CLIP词汇表中获得，但在TAD中是必要的。为了解决这个问题，我们学习了一个特定的背景嵌入，表示为Gbge∈RC'。我们将其附加到嵌入的动作类Gke中，使Flan成为包含K+1类的嵌入。背景嵌入Gbge是随机初始化的　　
...
![img](./assets/1859337-20220804194501867-26010444.png)
...
　　然后，我们在阈值θbin处对这个掩码进行二值化，并选择前景掩码，用<img src="./assets/1859337-20220809111841570-1137051985.png" alt="img" style="zoom:25%;" />表示。为了获得前景特征Ffgvis，我们使用<img src="https://img2022.cnblogs.com/blog/1859337/202208/1859337-20220809111841570-1137051985.png" alt="img" style="zoom: 25%;" />检索嵌入Fvis的片段。鉴于这个前景特征掩码<img src="https://img2022.cnblogs.com/blog/1859337/202208/1859337-20220809111841570-1137051985.png" alt="img" style="zoom:25%;" />的二进制性质，我们的表示掩码可以首先在见过的动作类上进行优化，并进一步推广到未见过的类。
...
　　**(I)上下文化视觉语言分类器**:在CLIP[35]的标准训练过程中，通常在对比对齐时使用全局特征。通常，它通过取片段特征的平均池化来估计片段-文本得分对，然后将其与语言特征一起使用。然而，该公式不适用于像TAD这样的密集分类任务，在这些任务中，每个时序片段都需要分配一个类标签。考虑到这一点，我们转而使用更新后的文本特征<img src="./assets/1859337-20220809112330953-127758383.png" alt="img" style="zoom:25%;" />∈R(K+1)×C和掩码前景特征Fvisfg∈RT×C:
...
　　我们进一步引入了由二元交叉熵(BCE)形成的一维动作完备性损失。它可以惩罚前景掩码输出<img src="./assets/1859337-20220809113404664-453429752.png" alt="img" style="zoom:25%;" />∈ RT×1。给定一个ground-truth one-hot前景掩码<img src="./assets/1859337-20220809113445358-1383410870.png" alt="img" style="zoom:25%;" />∈R，我们设计损失来建模前景的完备性如下:
...
　　其中<img src="./assets/1859337-20220809113623880-1022169412.png" alt="img" style="zoom:25%;" />= topk(argmax((Pbin∗Ep)[: K，:]))是从阈值分类得到的得分最高的前景片段中获得的特征，输出Pbin := η(p−θc)与θc 阈值，将嵌入E传递到一个1D conv层得到Ep，用于匹配p的维度。从掩码输出M中获得的最高评分特征类似于:<img src="./assets/1859337-20220809113925093-1964766499.png" alt="img" style="zoom: 25%;" />= topk(σ(1DPool(Em * Mbin)))，其中Mbin:= η(M−θm)是掩码预测M的二值化，Em通过将嵌入E传递到一维conv层以匹配维数M得到，σ为sigmoid激活函数。
